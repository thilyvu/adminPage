      ...
      <div class="row">
        <div class="col">
          ...
          >
            ...
            <el-table-column prop="fileSize" label="File Size" #default="scope">
              <p v-if="scope.row.fileSize">
                {{ `${scope.row.fileSize} kb` }}
              </p>
            </el-table-column>
            <el-table-column label="Thumbnail" #default="scope">
              <el-upload
                class="upload-demo"
                action="https://jsonplaceholder.typicode.com/posts/"
                :file-list="[
                  { name: scope.row.fileName, url: scope.row.thumbnailUrl },
                ]"
                :disabled="true"
                :thumbnail-mode="true"
              >
      ...
      >
        ...
        <el-form ref="form" :model="sizeForm" label-width="120px" size="mini">
          <el-form-item label="File Name">
            <el-input v-model="sizeForm.fileName"></el-input>
          </el-form-item>
          <el-form-item label="File Type">
            <el-select
              v-model="sizeForm.fileType"
              :disabled="true"
          ...
          <el-form-item label="File Size">
            <el-input v-model="sizeForm.fileSize"></el-input>
          </el-form-item>
          <el-form-item label="Thumbnail">
            <el-upload
              ...
              action="https://jsonplaceholder.typicode.com/posts/"
              :file-list="[
                { name: sizeForm.fileName, url: sizeForm.thumbnailUrl },
              ]"
              :disabled="true"
              :thumbnail-mode="true"
            >
...
export default {
  ...
  async mounted() {
    this.loading = true;
    Api.get("/getAllFileFolder", { params: { page: this.page, limit: 10 } })
      .then((res) => {
  ...
  methods: {
    ...
    handleDelete(index, row) {
      this.fileId = row._id;
      ElMessageBox.confirm("Are you sure to delete this file/folder?")
        .then(() => {
          this.loading_delete_btn;
          Api.delete(`/file/${this.fileId}`)
            .then(() => {
              this.$notify({
                type: "success",
                title: "Delete file/folder",
                message: "Delete file/folder successful",
              });
    ...
    onSubmit() {
      ...
      Api.put(`/file/${this.fileId}`, {
        fileName: this.sizeForm.fileName,
        // fileType: this.sizeForm.fileType,
        thumbnailUrl: this.sizeForm.thumbnailUrl,
        fileSize: this.sizeForm.fileSize,
      })
        .then(() => {
          this.$notify({
            type: "success",
            title: "Update file folder",
            message: "Update file folder successful",
          });
